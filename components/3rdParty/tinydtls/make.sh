...
# unescape the TARGET_ARGS variable:
ta=`eval echo $TARGET_ARGS`
...
ac_cv_func_malloc_0_nonnull=yes  ./configure \
    ...
    --host="$($CC -dumpmachine)" \
    CFLAGS="-w $ta -std=gnu99 -DSIERRA -DWITH_SHA256 ${PLATFORM_CFLAGS} ${CFLAGS}" \
    CPPFLAGS="-w $ta -std=gnu99 -DSIERRA -DWITH_SHA256 ${PLATFORM_CFLAGS} ${CFLAGS}" \
    LDFLAGS="${LDFLAGS}"
make -k
make -k install
